...
export default function LiveExtractor() {
  ...
  // If the person just uploaded or loaded a record on the Ingest page,
  // pick up that real text here instead of silently discarding it and
  // starting from the generic sample again.
  useEffect(() => {
    try {
      const passed = window.sessionStorage.getItem("nexus_ingest_text");
      if (passed) {
        setText(passed);
        setHandedOff(true);
        window.sessionStorage.removeItem("nexus_ingest_text");
      }
    } catch {
      // sessionStorage unavailable (e.g. private browsing) — fall back
      // to the built-in sample, nothing else to do here.
    }
  }, []);
    ...
    <Section id="extract">
      <Container>
        <Reveal>
          ...
          {handedOff && (
            <p className="mt-3 max-w-2xl text-sm text-[#ff3d00]">
              Loaded the record you selected on the Ingest page.
            </p>
          )}
        </Reveal>
